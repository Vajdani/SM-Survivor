...
local alertRenderableTp = "$CONTENT_DATA/Characters/Totebot/char_totebot_alert.rend"
local roamingRenderableTp = "$CONTENT_DATA/Characters/Totebot/char_totebot_roaming.rend"
...
function TotebotCharacter.cl_initGraphics( self )
	...
	self.character:setMovementEffects( "$CONTENT_DATA/Characters/empty_movement_effects.json" )
...
function TotebotCharacter.client_onUpdate( self, deltaTime )
	if not self.graphicsLoaded or not sm.exists( self.character ) then
		return
	end
...
function TotebotCharacter.cl_handleEvent( self, event )
	if not self.animationsLoaded or not sm.exists( self.character ) then
		return
	end
